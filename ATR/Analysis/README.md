...
- Then runs an additive `direction_split` pass for unmatched 2025 rows. This pass can match a single-direction 2025 count to one direction from a nearby bidirectional 2024 ATR segment when the 2025 direction is contained in the 2024 direction set and the same distance/text confidence tiers pass. These rows are flagged with `match_type=direction_split`, `needs_review=TRUE`, and `strict_reject_reason=direction_set_mismatch`.
- Uses direction-specific 2024 volumes for `direction_split` rows, not the full bidirectional 2024 total. This avoids double-counting while capturing partner-important sites such as Canal Street, Allen Street, Columbia Street, West 62 Street, and Major Deegan direction splits.
...
The matching process found 43 confident 2024-to-2025 rows: 32 strict one-to-one matches and 11 direction-split matches. Of those, 14 strict rows also have matched 2023 history. The 2024-to-2025 percent changes range from -82.23% to +54.88%; among rows with 2023 history, 2023-to-2025 percent changes range from -19.25% to +70.77%.
