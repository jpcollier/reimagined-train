...
- Then runs an additive `direction_split` pass for unmatched 2025 rows. This pass can match a single-direction 2025 count to a nearby 2024 ATR direction when the directions are either the same or are methodology-compatible non-opposites. For example, NB can be compatible with EB/WB if one methodology records the road axis and another records travel direction, but NB is not compatible with SB. These rows are flagged with `match_type=direction_split`, `needs_review=TRUE`, and `strict_reject_reason` values such as `direction_set_mismatch`, `direction_methodology_mismatch`, or `partner_full_value_override`.
- Uses direction-specific 2024 volumes for `direction_split` rows, not the full bidirectional 2024 total. This avoids double-counting while capturing partner-important sites such as Canal Street, Allen Street, Columbia Street, West 62 Street, Major Deegan, Ferry Terminal Drive, FDR Drive, 65 St Transverse, LIE Exit 15 WB, and direction-methodology mismatches like West 79 Street.
...
The matching process found 52 confident 2024-to-2025 rows: 32 strict one-to-one matches and 20 direction-split matches. Of those, 14 strict rows also have matched 2023 history. The 2024-to-2025 percent changes range from -82.23% to +658.56%; among rows with 2023 history, 2023-to-2025 percent changes range from -19.25% to +70.77%.
